...
using only the provided context. Anthropic is also supported as an
alternative provider (set ANTHROPIC_API_KEY instead). If neither key is set
(or the API call fails), `demo_answer` provides an extractive fallback: the
most relevant retrieved passages, verbatim, clearly labeled as demo mode so
it's never mistaken for a generated answer.
...
def detect_provider() -> Optional[str]:
    """Return 'openai', 'anthropic', or None based on which API key (if any)
    is set in the environment. OpenAI is preferred if both are set; Anthropic
    is supported as an alternative provider."""
...
def _call_anthropic(question: str, results: List[RetrievalResult]) -> str:
    """Alternative provider path, used when ANTHROPIC_API_KEY is set instead."""
...
def demo_answer(question: str, results: List[RetrievalResult]) -> str:
    ...
    lines = [
        "[DEMO MODE - no OPENAI_API_KEY / ANTHROPIC_API_KEY found in the "
