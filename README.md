...
- **LLM-synthesized answers when you want them.** Set `OPENAI_API_KEY` and
  ...
  answer grounded in (and citing) your documents.
   ...
   docqa/llm.py
      ...
      \-- OPENAI_API_KEY set
              --> generate_answer(): send retrieved chunks + question to
                  OpenAI via a plain HTTPS request, ask it to answer using
                  ONLY that context and cite sources
          |
          v
...
That's the only supported provider (via `gpt-4o-mini`; override with
`DOCQA_OPENAI_MODEL`). If the API call fails for any reason (bad key,
network error, rate limit), the tool automatically falls back to demo mode
...
------------------------------------------------------------------------
[DEMO MODE - no OPENAI_API_KEY found in the environment, so no answer is being generated by an LLM. Showing the most relevant passages retrieved from your documents instead.]
