...
- **LLM-synthesized answers when you want them.** Set `OPENAI_API_KEY` and
  the tool sends the retrieved chunks to GPT to generate a natural-language
  answer grounded in (and citing) your documents. (An alternative provider
  is also supported — see Setup below.)
...
```
 sample_docs/*.txt,*.pdf
   ...
   docqa/llm.py
      ...
      |
      \-- OPENAI_API_KEY set (or an alternative provider key)
              --> generate_answer(): send retrieved chunks + question to
                  the configured LLM provider via a plain HTTPS request, ask
                  it to answer using ONLY that context and cite sources
          |
          v
...
│   ├── llm.py            # optional OpenAI answer synthesis + demo mode
...
natural-language answers instead of returning raw passages, export an API
...
That's the default provider (via `gpt-4o-mini`; override with
`DOCQA_OPENAI_MODEL`). If you'd rather use Anthropic instead, set
`ANTHROPIC_API_KEY="sk-ant-..."` as an alternative — it's used automatically
whenever `OPENAI_API_KEY` isn't set (via `claude-3-5-haiku-20241022`,
overridable with `DOCQA_ANTHROPIC_MODEL`). If the API call fails for any
reason (bad key, network error, rate limit), the tool automatically falls
back to demo mode rather than crashing.
...
------------------------------------------------------------------------
[DEMO MODE - no OPENAI_API_KEY / ANTHROPIC_API_KEY found in the environment, so no answer is being generated by an LLM. Showing the most relevant passages retrieved from your documents instead.]
...
With an LLM API key set (see Setup above), the same commands instead return
a synthesized natural-language answer (e.g. *"Scout Mini has a battery life
of about 6 hours per charge (product_lineup.txt)."*) with a "Sources
consulted" list, rather than raw passages.
